...
export class HeaderComponent implements OnInit {
  ...
  salir() {
    ...
    // Definir la lógica para verificar si un permiso está presente en el array
    tienePermiso(permiso: string): boolean {
      return this.permisos!.some(p => p.name === permiso);
    }
    ...
    permisoListUser(): boolean {
      return this.permisos?.some(p => p.name === 'LIST_USER') ?? false;
    }
    ...
    permisoUpdateMeasureUnits(): boolean {
      return this.permisos?.some(p => p.name === 'UPDATE_MEASURE_UNITS') ?? false;
    }
    ...
    permisoDeleteMeasureUnits(): boolean {
      return this.permisos?.some(p => p.name === 'DELETE_MEASURE_UNITS') ?? false;
    }
    ...
    permisoDeleteProduction(): boolean {
      return this.permisos?.some(p => p.name === 'DELETE_PRODUCTION') ?? false;
    }
